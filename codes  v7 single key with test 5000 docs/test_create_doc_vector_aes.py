...
set_of_words = set()
input_file = open('dictionary_for_aes.txt','r')
words_from_dict_file = []
for readline in input_file :
    line = readline.strip()
    for i in (line.split('\t') ) :
        words_from_dict_file.append(i)
#print(words_from_dict_file)
input_file.close()

for i in words_from_dict_file :
    set_of_words.add(i)

df = pd.DataFrame(columns = set_of_words)
#print(df)


input_df = pd.read_csv("test_cipher_texts.csv")
#print(input_df.head())
cipher_texts = input_df.iloc[:,1]
#print(cipher_texts.shape)
...
for cipher_text in cipher_texts :
    ...
    for i in set_of_words :
        if i not in  dictionary_of_words.keys() :
            ...
            #print(dictionary_of_words.keys())
    for i in dictionary_of_words.keys() :
        for j in list_of_words :
            if i == j :
                dictionary_of_words[j] += 1
    #print(dictionary_of_words)
    words = dictionary_of_words.keys()
    frequency = dictionary_of_words.values()
    #print(set_of_words)

    df_row = pd.DataFrame([dictionary_of_words])
    #print(df_row.head())
    df = pd.concat([df, df_row], ignore_index = True, axis = 0)

    #print(dictionary_of_words)
print(df.shape)
df.to_csv('test_doc_vector_for_aes.csv')
